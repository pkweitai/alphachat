...
@functools.lru_cache
def user_alpha_chat_template() -> SystemMessagePromptTemplate:
    with open(join(template_path(), "user_alphaservices.md"), "r") as fd:
        return SystemMessagePromptTemplate.from_template(fd.read())

@functools.lru_cache
def user_action_detect() -> SystemMessagePromptTemplate:
    with open(join(template_path(), "user_action_classification.md"), "r") as fd:
        return SystemMessagePromptTemplate.from_template(fd.read())
